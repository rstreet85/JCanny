...
public class Gaussian {
    ...
    /*
     * Blurs a single-channel image
     */
    public static double[][] BlurDouble (double[][] raw, int rad, double intens) {
        if (raw.length < 2 * rad + 1 || raw[0].length < 2 * rad + 1) {
            throw new IllegalArgumentException("Image is smaller than the Gaussian mask!");
        }
        
        if (rad <= 0 || intens <= 0) {
            throw new IllegalArgumentException("Mask radius/intensity cannot be 0!");
        }
        
        double[] mask = new double[2 * rad + 1];
        double norm = 0.;
        double[][] outSing = new double[raw.length - 2 * rad][raw[0].length - 2 * rad];
